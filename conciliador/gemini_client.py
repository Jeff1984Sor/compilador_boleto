...
Aceita PDF diretamente (sem precisar converter para imagem como na OpenAI).
"""
...
_PROMPT_COMPROVANTE = """\
Voce esta lendo um COMPROVANTE DE PAGAMENTO brasileiro (boleto bancario OU PIX) em PDF (1 pagina).
Extraia os campos abaixo e devolva APENAS um JSON valido (sem markdown, sem texto antes ou depois).
...
def _parse_json(texto: str) -> dict:
    """Tolera ```json ... ``` fences que alguns modelos devolvem."""
    t = texto.strip()
    if t.startswith("```"):
        t = re.sub(r"^```(?:json)?\s*", "", t)
        t = re.sub(r"\s*```$", "", t)
